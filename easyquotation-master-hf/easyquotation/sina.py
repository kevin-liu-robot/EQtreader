...
class Sina(basequotation.BaseQuotation):
    ...
    def get_sina_lhb1(self):
        response = requests.get("https://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHQNodeData?page=1&num=80&sort=changepercent&asc=0&node=hs_a&symbol=&_s_r_a=page")
        stock_dict = self.format_response_lhb(response.text, prefix=True)        
        return stock_dict

    def get_sina_lhb2(self):
        response = requests.get("https://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHQNodeData?page=2&num=80&sort=changepercent&asc=0&node=hs_a&symbol=&_s_r_a=page")
        stock_dict = self.format_response_lhb(response.text, prefix=True)
        return stock_dict

    def get_sina_lhb3(self):
        response = requests.get("https://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHQNodeData?page=3&num=80&sort=changepercent&asc=0&node=hs_a&symbol=&_s_r_a=page")
        stock_dict = self.format_response_lhb(response.text, prefix=True)
        return stock_dict
    ...
    def get_0915(self,stock_list):
        ...
        return self.format_response_data(response.text, prefix=False)
    ...
    def format_response_lhb(self, rep_data, prefix=False):
        stocks_detail = "".join(rep_data)
        stock_dict = json.loads(stocks_detail)
        return stock_dict
