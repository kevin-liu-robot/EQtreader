...
class ClientTrader(IClientTrader):
    ...
    def market_trade(self, security, amount, ttype=None, limit_price=None, **kwargs):
        ...
        self._clickpaste_str(self._config.TRADE_SECURITY_CONTROL_ID,code)
        if ttype is not None:
            ...
            while retry < retry_max:
                try:
                    ...
                    retry += 1
                    self.wait(0.1)
        self._set_market_trade_params_paste(security, amount, limit_price=limit_price)
        self._submit_trade_click()
        #self._submit_trade()
    ...
    def trade(self, security, price, amount):
        self._set_trade_params_paste(security, price, amount)
    ...
    @perf_clock
    def _submit_trade_click(self):
        time.sleep(0.1)
        element_position = self._main.child_window(
        ...
        pywinauto.mouse.click(coords=center_position)
    ...
    def _clickpaste_str(self, control_id, content):
        self.Copy_To_Clipboard(content)
        self._main.child_window(
                control_id=control_id, class_name="Edit"
            ).click()
        pywinauto.keyboard.SendKeys("{DELETE}")
        pywinauto.keyboard.SendKeys("{DELETE}")
        pywinauto.keyboard.SendKeys("{DELETE}")
        pywinauto.keyboard.SendKeys("{DELETE}")
        pywinauto.keyboard.SendKeys("{DELETE}")
        ...
        center_position = (int((element_position.left + element_position.right) / 2),
                           int((element_position.top + element_position.bottom) / 2))
        paste_d = int((element_position.bottom - element_position.top) / 2 * 10)
        paste_position =  (int(element_position.right),
                           int((element_position.top + element_position.bottom) / 2)+paste_d)        
        pywinauto.mouse.right_click(coords=center_position)
        pywinauto.mouse.click(coords=paste_position)

        
    def _set_trade_params_paste(self, security, price, amount):
        code = security[-6:]
        ...
        self._clickpaste_str(self._config.TRADE_PRICE_CONTROL_ID,price)

        '''self._type_edit_control_keys(
            self._config.TRADE_AMOUNT_CONTROL_ID, str(int(amount))
        )'''
        self._clickpaste_str(self._config.TRADE_AMOUNT_CONTROL_ID,str(int(amount)))
        
    def _set_market_trade_params_paste(self, security, amount, limit_price=None):
        self._clickpaste_str(self._config.TRADE_AMOUNT_CONTROL_ID,str(int(amount)))
